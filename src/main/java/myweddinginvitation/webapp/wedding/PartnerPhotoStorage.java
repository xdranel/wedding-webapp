...
public class PartnerPhotoStorage {
	...
	public String store(MultipartFile file) {
		if (file == null || file.isEmpty() || file.getSize() > MAX_SIZE) throw new IllegalArgumentException("Photo must be at most 10 MiB");
	...
	private PhotoType photoType(InputStream input) throws IOException {
		...
		if (matchesAt(header, 0, 0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a)) return new PhotoType(".png", MediaType.IMAGE_PNG);
		if (matchesAt(header, 0, 'R', 'I', 'F', 'F') && matchesAt(header, 8, 'W', 'E', 'B', 'P')) return new PhotoType(".webp", MediaType.valueOf("image/webp"));
